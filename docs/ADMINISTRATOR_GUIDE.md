...
**Таймзона сервиса** (`/onboarding`, `BOT_TIMEZONE`): используется для расписания бота, времени в тестовом сообщении пользователя из панели и для `%(asctime)s` в логах бота и админки после перезапуска.
