...
1. **`tpl_test_message`** — отдельный шаблон для тестовой отправки из админки (`/users/test-message`, `/groups/test-message`), не смешивается с рабочими event-шаблонами.
2. **`tpl_dry_run`** — выведен из runtime-контракта (удалён из реестра, API и шаблонов); любые старые override-строки считаются orphan и очищаются миграцией.
3. **`tpl_digest`** — удалён из продуктового контура (реестр/API/шаблон на диске).
