...
| Интервалы, таймзона бота, шаблоны отчётов и уведомлений, **Matrix device ID** | `/settings` (вкладки), [`/onboarding`](../src/admin/routes/settings.py), API [`/api/bot/content`](../src/admin/routes/bot_content.py) | `cycle_settings` (`CycleSettings`); ключи в т.ч. `BOT_TIMEZONE`, `MATRIX_DEVICE_ID` | [`load_catalogs`](../src/bot/catalogs.py) + [`fetch_cycle_settings`](../src/database/load_config.py) в [`main.py`](../src/bot/main.py) |
