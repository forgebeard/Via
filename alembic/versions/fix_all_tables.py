...
def upgrade() -> None:
    ...
    op.add_column(
        "bot_users", sa.Column("versions", sa.JSON(), nullable=False, server_default='["all"]')
    )
    op.add_column(
        "bot_users", sa.Column("priorities", sa.JSON(), nullable=False, server_default='["all"]')
    )

    # Add columns to support_groups if not exists
    op.add_column(
        "support_groups", sa.Column("versions", sa.JSON(), nullable=False, server_default='["all"]')
    )
    op.add_column(
        "support_groups",
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_redmine_priorities_redmine_priority_id"), table_name="redmine_priorities"
    )
