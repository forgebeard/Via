...
def downgrade() -> None:
    # Re-add columns
    op.add_column("bot_users", sa.Column("redmine_api_key_ciphertext", sa.LargeBinary(), nullable=True))
    op.add_column("bot_users", sa.Column("redmine_api_key_nonce", sa.LargeBinary(), nullable=True))
    op.add_column("bot_users", sa.Column("redmine_api_key_key_version", sa.SmallInteger(), nullable=False, server_default="1"))
    ...
    op.create_table(
        ...
        sa.Column("existing_bot_user_id", sa.Integer(), sa.ForeignKey("bot_users.id", ondelete="SET NULL"), nullable=True),
